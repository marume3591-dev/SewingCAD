...
enum StandardBodyGenerator {
    ...
    private static func buildTorso(
        ...
        let slices: [(y: Float, rx: Float, rz: Float, region: BodyRegion, w: Float)] = [
            ...
            // ウエスト→ヒップ：前後方向(rz)を抑えてお腹の突き出しを防ぐ
            (105, 15.9,  8.5,  .abdomen,   0.75),  // rzをウエストと同水準に維持
            (102, 16.8,  8.6,  .abdomen,   0.75),  // 徐々に広げる
            (100, 18.0,  8.8,  .abdomen,   0.80),  // rzの増加を最小限に
            ( 99, 19.2,  9.2,  .hip,       0.85),  // ヒップへ移行
            ( 96, 21.0,  9.8,  .hip,       1.0),   // ヒップ最大（横幅を抑制）
            ( 93, 20.5,  9.5,  .hip,       0.95),
