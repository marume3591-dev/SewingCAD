...
enum StandardBodyGenerator {
    ...
    private static func buildTorso(
        ...
        let slices: [(y: Float, rx: Float, rz: Float, region: BodyRegion, w: Float)] = [
            ...
            // ウエスト→ヒップ：rzはウエストとほぼ同値に固定（腰の前後突き出し防止）
            (105, 15.9,  8.3,  .abdomen,   0.75),  // rzをウエストより若干抑制
            (102, 16.8,  8.3,  .abdomen,   0.75),  // rz変化なし
            (100, 18.0,  8.4,  .abdomen,   0.80),  // rz微増のみ
            ( 99, 19.2,  8.5,  .hip,       0.85),  // ヒップ開始、rzはウエスト同水準
            ( 96, 21.0,  8.8,  .hip,       1.0),   // ヒップ最大、rzを9.8→8.8に大幅削減
            ( 93, 20.5,  8.6,  .hip,       0.95),
            ( 90, 19.5,  8.4,  .hip,       0.85),
