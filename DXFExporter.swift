...
class DXFExporter {
    ...
    private static func buildDXF(canvasState: CanvasState) -> String {
        // 1px = 1/37.8 cm → DXFはmm単位が一般的なので 1px = 10/37.8 mm
        ...
        func pxD(_ v: Double) -> String {
            String(format: "%.4f", v * pxToMm)
        }
        ...
        for notch in canvasState.notches {
            ...
            let sz: CGFloat = notch.size   // px単位

            let tip   = pos
            let leftX  = pos.x + nx * sz - tx * sz * 0.6
            let leftY  = pos.y + ny * sz - ty * sz * 0.6
            let rightX = pos.x + nx * sz + tx * sz * 0.6
            let rightY = pos.y + ny * sz + ty * sz * 0.6
            let left   = CGPoint(x: leftX,  y: leftY)
            let right  = CGPoint(x: rightX, y: rightY)
